...
{
  unsigned int nb_Byte, nb_Byte_adress, line = 1, line_code, adress = 0, sum = 0;
  ...
  do
  {
    ...
    nb_Byte_adress = nb_Byte_per_adress(line_code);

    // Check if the 'S' is followed by 1, 2, 3, 7, 8 or 9
    if (nb_Byte_adress == 0)
      error = 1;

    nb_Byte = retrieve_nb_Byte(file, &sum); // Calculate number of bytes for data
    adress = retrieve_adress(file, &sum, nb_Byte_adress); // Calculate adress

    // Check if adress is not out of memory
    if (adress < SIZE_MEMORY)
      put_data_in_memory(file, memory, &sum, nb_Byte, nb_Byte_adress, adress);
    else
      error = 1;
    ...
    // Check if the character is end of line
    ...
    // Is it end of line or end of file ? If it is end of line, we continue else we stop
